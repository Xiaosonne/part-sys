        ...
        <el-table-column label="锁定来源" min-width="320">
          <template #default="{ row }">
            <div v-for="lock in (row.locks || []).slice(0, 2)" :key="lock.transactionId" class="lock-source-item">
              <span class="project-tag">{{ lock.projectName || '未知项目' }}</span>
...
const pagedLocks = computed(() => {
  ...
  return filteredLocks.value.slice(start, start + pageSize.value)
})
...
const loadData = async () => {
  ...
  try {
    ...
    // 数据已经包含 locks 数组，直接使用
    allLocks.value = (res.data || []).map(p => ({ ...p }))
  } catch (error) {
